...
class Solution:
    ...
    where ii is the slow-runner while j is the fast-runner.
    As long as nums[i] = nums[j], we increment j to skip the duplicate.

    When we encounter nums[j] not = nums[i], the duplicate run has ended so we must copy its value to nums[i + 1]. 
...
class removeDuplicatesCase(unittest.TestCase):
    ...
    def test_removeDuplicates(self):
        ...
        self.assertEqual(func([1,1,2]), (2, [1,2]))
        self.assertEqual(func([0,0,1,1,1,2,2,3,3,4]), (5, [0,1,2,3,4]))
